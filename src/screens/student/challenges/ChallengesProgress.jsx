...
export function ChallengesProgress() {
  ...
  return (
    <section className="relative mx-auto w-full max-w-[440px] bg-[#0c120c] font-sans min-h-[100dvh] flex flex-col justify-center overflow-x-hidden overflow-y-auto">
      
      {/* ───── Header (Back Button) - Fixed to top of viewport ───── */}
      ...
      {/* ───── Map Container (Maintains Aspect Ratio tracking) ───── */}
      <div className="relative w-full">
        ...
        {/* Overlays for the nodes - Pinned exactly to the image dimensions */}
        <div className="absolute top-0 left-0 w-full h-full z-10">
          ...
          <div className="absolute bottom-[8%] left-6 pointer-events-none drop-shadow-2xl font-serif">
            <h2 className="text-[clamp(42px,12vw,52px)] leading-[1.1] text-white tracking-wide">
              Growth
            </h2>
            <div className="flex items-baseline gap-2 mt-[-5px]">
              <span className="text-[clamp(32px,9vw,40px)] leading-none text-white tracking-wide">is a</span>
              <span className="text-[clamp(42px,12vw,52px)] leading-[1.1] text-[#eab308] tracking-wide">path</span>
            </div>
          </div>
        </div>
      </div>
...
function JourneyNode({ num, title, status, img, active, color, onClick }) {
  return (
    ...
    >
      ...
      <div 
        className={`relative w-[clamp(90px,25vw,110px)] h-[clamp(95px,26vw,115px)] rounded-[18px] overflow-hidden transition-all ${
          active 
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 pb-3 flex flex-col items-center">
          <p className="text-[clamp(11px,3.5vw,13px)] font-bold text-white drop-shadow-md tracking-wide">
            {title}
          </p>
      ...
      <div 
        className="absolute -left-3 top-1/2 -translate-y-1/2 size-[clamp(20px,6vw,24px)] rounded-full flex items-center justify-center border-[1.5px] border-black/90 shadow-md z-10"
        style={{ backgroundColor: color }}
      >
        <span className="text-[clamp(8px,2.5vw,9px)] font-black text-white leading-none">{num}</span>
      </div>
    </motion.button>
  );
}
